<?php $a1_class='main';$a1_title='MENU_INDEX_PROJECTMENU'; ?><?php unset($a1_class,$a1_title) ?><?php $a2_title='GLOBAL_PROJECTS';$a2_name='login';$a2_icon='project';$a2_width='600';$a2_rowclasses='odd,even';$a2_columnclasses='1,2,3'; ?><div class="breadcrumb">
		<?php 
	$icon=$a2_icon;
...
<?php } ?>
 colspan="3"
><?php unset($a5_colspan) ?><?php $a6_title=$name; ?><fieldset><?php if(isset($a6_title)) { ?><legend><?php if(isset($a6_icon)) { ?><image src="<?php echo $image_dir.'icon_'.$a6_icon.IMG_ICON_EXT ?>" align="left" border="0"><?php } ?><?php echo encodeHtml($a6_title) ?></legend><?php } ?><?php unset($a6_title) ?></fieldset></td></tr><?php
	$row_idx++;
	$column_idx = 0;
?>
<tr
...
<?php if (!empty($column_classes)) { ?>
 class="<?php echo $column_classes[($column_idx-1)%count($column_classes)] ?>"
<?php } ?>
><?php $a6_title=lang('TREE_CHOOSE_PROJECT');$a6_url=$url;$a6_class=''; ?><?php
	$params = array();
	$tmp_url = '';
		$tmp_url = $a6_url;
?><a<?php if (isset($a6_name)) echo ' name="'.$a6_name.'"'; else echo ' href="'.$tmp_url.(isset($a6_anchor)?'#'.$a6_anchor:'').'"' ?> class="<?php echo $a6_class ?>"<?php if (isset($a6_accesskey)) echo ' accesskey="'.$a6_accesskey.'"' ?>  title="<?php echo encodeHtml($a6_title) ?>"><?php unset($a6_title,$a6_url,$a6_class) ?><?php $a7_var='project';$a7_value='project'; ?><?php
	if (isset($a7_key))
