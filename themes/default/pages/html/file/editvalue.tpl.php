...
?><td <?php foreach( $attr as $a_name=>$a_value ) echo " $a_name=\"$a_value\"" ?>><?php unset($attr) ?><?php unset($attr_class) ?><?php $attr = array('class'=>'text','text'=>'GLOBAL_VALUE') ?><?php $attr_class='text' ?><?php $attr_text='GLOBAL_VALUE' ?><?php
	if(empty($attr_title))
		if (!empty($attr_key))
...
?><td <?php foreach( $attr as $a_name=>$a_value ) echo " $a_name=\"$a_value\"" ?>><?php unset($attr) ?><?php unset($attr_class) ?><?php $attr = array('name'=>'value','rows'=>'35','cols'=>'60','class'=>'') ?><?php $attr_name='value' ?><?php $attr_rows='35' ?><?php $attr_cols='60' ?><?php $attr_class='' ?><textarea class="<?php echo $attr_class ?>" name="<?php echo $attr_name ?>" rows="<?php echo $attr_rows ?>" cols="<?php echo $attr_cols ?>"><?php echo htmlentities(isset($$attr_name)?$$attr_name:$attr_default) ?></textarea><?php unset($attr) ?><?php unset($attr_name) ?><?php unset($attr_rows) ?><?php unset($attr_cols) ?><?php unset($attr_class) ?><?php $attr = array() ?></td><?php unset($attr) ?><?php $attr = array() ?></tr><?php unset($attr) ?><?php $attr = array() ?><?php
	$row_class_idx++;
	if ($row_class_idx > count($row_classes))
		$row_class_idx=1;
	$row_class=$row_classes[$row_class_idx-1];
...
<?php unset($attr) ?><?php $attr = array('field'=>'value') ?><?php $attr_field='value' ?>
