...
<?php } ?>
 colspan="2"
><?php unset($a6_colspan,$a6_header) ?><?php $a7_title=$projectname; ?><fieldset><?php if(isset($a7_title)) { ?><legend><?php if(isset($a7_icon)) { ?><img src="<?php echo $image_dir.'icon_'.$a7_icon.IMG_ICON_EXT ?>" align="left" border="0" /><?php } ?>&nbsp;&nbsp;<?php echo encodeHtml($a7_title) ?>&nbsp;&nbsp;</legend><?php } ?><?php unset($a7_title) ?><?php $a8_empty='acls'; ?><?php 
	if	( !isset($$a8_empty) )
	...
	{
?>
<?php unset($a8_empty) ?><div><?php $a10_class='text';$a10_text='GLOBAL_NOT_FOUND';$a10_escape=true;$a10_cut='both'; ?><?php
		$a10_title = '';
		$tmp_tag = 'span';
?><<?php echo $tmp_tag ?> class="<?php echo $a10_class ?>" title="<?php echo $a10_title ?>"><?php
	...
	echo $tmp_text;
	unset($tmp_text);
?></<?php echo $tmp_tag ?>><?php unset($a10_class,$a10_text,$a10_escape,$a10_cut) ?></div><?php } ?><?php $a8_not=true;$a8_empty='acls'; ?><?php 
	if	( !isset($$a8_empty) )
		$a8_tmp_exec = empty($a8_empty);
	elseif	( is_array($$a8_empty) )
		$a8_tmp_exec = (count($$a8_empty)==0);
...
<?php if (!empty($column_classes)) { ?>
 class="<?php echo $column_classes[($column_idx-1)%count($column_classes)] ?>"
<?php } ?>
><?php unset($a12_header) ?><?php $a13_present='username'; ?><?php 
	$a13_tmp_exec = isset($$a13_present);
	$a13_tmp_last_exec = $a13_tmp_exec;
	if	( $a13_tmp_exec )
	{
?>
<?php unset($a13_present) ?><?php $a14_align='left';$a14_type='user'; ?><?php
	$a14_tmp_image_file = $image_dir.'icon_'.$a14_type.IMG_ICON_EXT;
	$a14_size = '16x16';
	$a14_tmp_title = basename($a14_tmp_image_file);
?><img alt="<?php echo $a14_tmp_title; if (isset($a14_size)) { echo ' ('; list($a14_tmp_width,$a14_tmp_height)=explode('x',$a14_size);echo $a14_tmp_width.'x'.$a14_tmp_height; echo')';} ?>" src="<?php echo $a14_tmp_image_file ?>" border="0"<?php if(isset($a14_align)) echo ' align="'.$a14_align.'"' ?><?php if (isset($a14_size)) { list($a14_tmp_width,$a14_tmp_height)=explode('x',$a14_size);echo ' width="'.$a14_tmp_width.'" height="'.$a14_tmp_height.'"';} ?> /><?php unset($a14_align,$a14_type) ?><?php $a14_class='text';$a14_var='username';$a14_maxlength='20';$a14_escape=true;$a14_cut='both'; ?><?php
		$a14_title = '';
		$tmp_tag = 'span';
?><<?php echo $tmp_tag ?> class="<?php echo $a14_class ?>" title="<?php echo $a14_title ?>"><?php
	...
	echo $tmp_text;
	unset($tmp_text);
?></<?php echo $tmp_tag ?>><?php unset($a14_class,$a14_var,$a14_maxlength,$a14_escape,$a14_cut) ?><?php } ?><?php $a13_present='groupname'; ?><?php 
	$a13_tmp_exec = isset($$a13_present);
	$a13_tmp_last_exec = $a13_tmp_exec;
	if	( $a13_tmp_exec )
	{
