-- OpenRat DDL for Oracle RDBMS
--
-- Special Oracle behavour need the following differences:
--   using CLOBs for text an file values
--   keywords number, size, date in '"'
--   removed: "on delete restrict on update restrict"
--   ' ' instead of '' because of Oracles NULL/''-Bug
...
CREATE TABLE or_project (
     ...
     , ftp_passive         INT   DEFAULT 0 NOT NULL
     , cmd_after_publish   VARCHAR(255)           NOT NULL
     , content_negotiation INT   DEFAULT 0 NOT NULL
     , cut_index           INT   DEFAULT 0 NOT NULL
     , PRIMARY KEY (id)
);
...
CREATE TABLE or_page (
     ...
     , CONSTRAINT fk_page_01 FOREIGN KEY (templateid)
                  REFERENCES or_template (id) 
     , CONSTRAINT fk_page_02 FOREIGN KEY (objectid)
                  REFERENCES or_object (id) 
);
...
CREATE TABLE or_value (
     ...
     , CONSTRAINT fk_value_01 FOREIGN KEY (pageid)
                  REFERENCES or_page (id) 
     , CONSTRAINT fk_value_02 FOREIGN KEY (elementid)
                  REFERENCES or_element (id) 
