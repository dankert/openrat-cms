		...
		<form name="" target="_self" data-target="view" action="./" data-method="result" data-action="search" data-id="<?php echo OR_ID ?>" method="GET" enctype="application/x-www-form-urlencoded" class="search" data-async="" data-autosave=""><input type="submit" class="invisible" /><input type="hidden" name="<?php echo REQ_PARAM_EMBED ?>" value="1" /><input type="hidden" name="<?php echo REQ_PARAM_TOKEN ?>" value="<?php echo token() ?>" /><input type="hidden" name="<?php echo REQ_PARAM_ACTION ?>" value="search" /><input type="hidden" name="<?php echo REQ_PARAM_SUBACTION ?>" value="result" /><input type="hidden" name="<?php echo REQ_PARAM_ID ?>" value="<?php echo OR_ID ?>" />
			...
			<div class="line">
				...
				<div class="input">
					<?php { $tmpname     = 'id';$default  = config('search','quicksearch','flag','id');$readonly = '';		
		if	( isset($$tmpname) )
			$checked = $$tmpname;
		else
			$checked = $default;
		...
		?><input type="hidden" name="<?php echo $tmpname ?>" value="1" /><?php
					...
					<?php { $tmpname     = 'name';$default  = config('search','quicksearch','flag','name');$readonly = '';		
		if	( isset($$tmpname) )
			$checked = $$tmpname;
		else
			$checked = $default;
		...
		?><input type="hidden" name="<?php echo $tmpname ?>" value="1" /><?php
					...
					<?php { $tmpname     = 'filename';$default  = config('search','quicksearch','flag','filename');$readonly = '';		
		if	( isset($$tmpname) )
			$checked = $$tmpname;
		else
			$checked = $default;
		...
		?><input type="hidden" name="<?php echo $tmpname ?>" value="1" /><?php
					...
					<?php { $tmpname     = 'description';$default  = config('search','quicksearch','flag','description');$readonly = '';		
		if	( isset($$tmpname) )
			$checked = $$tmpname;
		else
			$checked = $default;
		...
		?><input type="hidden" name="<?php echo $tmpname ?>" value="1" /><?php
					...
					<?php { $tmpname     = 'content';$default  = config('search','quicksearch','flag','content');$readonly = '';		
		if	( isset($$tmpname) )
			$checked = $$tmpname;
		else
			$checked = $default;
