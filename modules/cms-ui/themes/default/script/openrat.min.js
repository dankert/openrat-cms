...
else if(Notification.permission!=='denied'){Notification.requestPermission(function(e){if(e==='granted'){var i=new Notification(t)}})}};function setUserStyle(t){var e=$('html'),i=e.attr('class').split(/\s+/);$.each(i,function(t,i){if(i.startsWith('theme-')){e.removeClass(i)}});e.addClass('theme-'+t.toLowerCase())};function insert(t,e,i){var a=document.forms[0].elements[t];a.focus();if(typeof document.selection!='undefined'){var r=document.selection.createRange(),n=r.text;r.text=e+n+i;r=document.selection.createRange();if(n.length==0){r.move('character',-i.length)}
else{r.moveStart('character',e.length+n.length+i.length)};r.select()}
else if(typeof a.selectionStart!='undefined'){var d=a.selectionStart,c=a.selectionEnd,n=a.value.substring(d,c);a.value=a.value.substr(0,d)+e+n+i+a.value.substr(c);var o;if(n.length==0){o=d+e.length}
else{o=d+e.length+n.length+i.length};a.selectionStart=o;a.selectionEnd=o}
else{o=a.value.length;var n=prompt('Text');a.value=a.value.substr(0,o)+e+n+i+a.value.substr(o)}};function createUrl(t,e,i,a,o){var n='./';n+='?action='+t;if(e!=null)n+='&subaction='+e;n+='&id='+i;if(o)n+='&embed=1';if(typeof a==='string'){a=a.replace(/'/g,'"');jQuery.each(jQuery.parseJSON(a),function(t,e){if(t=='action'||t=='subaction'||t=='id')return;n=n+'&'+t+'='+e})}
else if(typeof a==='object'){jQuery.each(a,function(t,e){if(t=='action'||t=='subaction'||t=='id')return;n=n+'&'+t+'='+e})}
