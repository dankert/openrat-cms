        ...
        <table width="100%">
          ...
          <?php foreach($el as $list_key=>$list_value) { extract($list_value); ?>
            <tr class="data">
              <td data-name="<?php echo encodeHtml(htmlentities(@$name)) ?>" data-action="group" data-id="<?php echo encodeHtml(htmlentities(@$id)) ?>" class="clickable clickable">
                <a target="_self" date-name="<?php echo encodeHtml(htmlentities(@$name)) ?>" name="<?php echo encodeHtml(htmlentities(@$name)) ?>" data-type="open" data-action="group" data-method="" data-id="<?php echo encodeHtml(htmlentities(@$id)) ?>" data-extra="[]" href="/#/group/<?php echo encodeHtml(htmlentities(@$id)) ?>">
          ...
          <tr class="data">
            <td colspan="2" class="clickable">
              <a target="_self" date-name="<?php echo encodeHtml(htmlentities(@lang('menu_add'))) ?>" name="<?php echo encodeHtml(htmlentities(@lang('menu_add'))) ?>" data-type="dialog" data-action="" data-method="add" data-id="" data-extra="{'dialogAction':null,'dialogMethod':'add'}" href="/#//">
